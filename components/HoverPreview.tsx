...
export default function HoverPreview() {
  ...
  useEffect(() => {
    ...
    const show = (target: HTMLElement, immediate: boolean) => {
      ...
      current = target;
      const render = () => {
        const project = target.hasAttribute("data-preview-title");
        const link = project ? target.querySelector("a") : target.closest("a");
        ...
        setPreview({
          title: target.dataset.previewTitle || link?.getAttribute("aria-label") || link?.textContent?.trim() || "Open link",
          description: target.dataset.previewDescription || (url?.protocol === "mailto:" ? "Start a conversation by email." : url?.pathname === "/resume.pdf" ? "View my résumé as a PDF." : url?.origin === location.origin ? (url.pathname === "/projects" ? "Browse all projects and experiments." : "Back to my portfolio.") : "Open in a new tab."),
          detail: target.dataset.previewDetail || (url?.protocol === "mailto:" ? "Email" : url?.origin === location.origin ? url.pathname : url?.hostname || "Project"),
          project,
          width,
    ...
    const targetFor = (node: EventTarget | null) => node instanceof Element ? node.closest<HTMLElement>("[data-preview-title], a") : null;
    const over = (event: PointerEvent) => {
      if (event.pointerType === "touch") return;
      if (panel.current?.contains(event.target as Node)) { clearTimeout(hideTimer); return; }
      const target = targetFor(event.target);
      if (target) show(target.closest<HTMLElement>("[data-preview-title]") ?? target, false);
    };
    ...
    const focus = (event: FocusEvent) => {
      const target = targetFor(event.target);
      if (target) show(target.closest<HTMLElement>("[data-preview-title]") ?? target, true);
      else close();
    };
    ...
    <AnimatePresence>
      {preview && (
        <motion.div ref={panel} id="hover-preview" role="tooltip" className="preview-window"
          ...
          <div className="preview-titlebar">{preview.project ? <FolderOpen size={13} /> : <FileText size={13} />}<span>{preview.title}</span><span className="ml-auto opacity-70" aria-hidden="true">↗</span></div>
