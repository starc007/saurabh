  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        "border border-edge bg-canvas/20 backdrop-blur-xl backdrop-saturate-150",
    ...
    >
      {active ? (
        <motion.span
          ...
          className="absolute inset-0.5 -z-10 rounded-xl bg-ink/5"
        />
      ) : null}
      {children}
    </motion.div>
...
export function DockSeparator({ className }: { className?: string }) {
  return (
    <span className={cn("mx-1 h-6 w-px self-center bg-edge", className)} />
  );
}
